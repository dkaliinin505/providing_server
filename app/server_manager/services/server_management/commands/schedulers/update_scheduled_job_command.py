...
class UpdateScheduledJobCommand(Command):
    ...
    async def execute(self, data, job_action='create'):
        ...
        self.job_id = self.config.get('job_id', None)  # Only for update
        ...
        cron_command = f'(crontab -l -u {self.user} | grep -v "# JOB_ID={self.job_id}"; echo "{self.frequency} {full_command} # JOB_ID={self.job_id}") | crontab -u {self.user} -'

        result = await run_command_async(cron_command, capture_output=True)

        if result:
            return {"message": f"Scheduled job updated successfully"}
        else:
            return {"error": f"Failed to updated scheduled job"}
